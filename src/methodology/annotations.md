...
description: "Annotations are a documentation-only overlay used on design system pages to surface accessibility concerns, behavioral rules, event tracking details, and responsive considerations directly alongside live component demos."
layout: base.njk
---
...
Each annotation belongs to one of four categories, identified by color and icon:

| Category | Icon | Color | Use for |
| --- | --- | --- | --- |
| Accessibility | <i class="fa-solid fa-universal-access" aria-hidden="true"></i> | <span class="annotation-swatch annotation-swatch--a11y">#0aa2c0</span> | ARIA attributes, heading hierarchy, label association, color contrast, screen reader concerns |
| Behavior | <i class="fa-solid fa-hand-pointer" aria-hidden="true"></i> | <span class="annotation-swatch annotation-swatch--behavior">#6f42c1</span> | UI rules, interaction constraints (e.g. "one primary button per view"), state management |
| Tracking | <i class="fa-solid fa-chart-simple" aria-hidden="true"></i> | <span class="annotation-swatch annotation-swatch--tracking">#e8590c</span> | GA4 event tracking attributes, event type inference, position tracking |
| Responsive | <i class="fa-solid fa-mobile-screen" aria-hidden="true"></i> | <span class="annotation-swatch annotation-swatch--responsive">#2f9e44</span> | Breakpoint behavior, scrollable wrappers, stacking order on mobile |
...
| `data-category` | Yes | One of `a11y`, `behavior`, `tracking`, `responsive`. |
